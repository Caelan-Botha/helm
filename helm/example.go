...
func HelloMainFunc(t *Helm, subCommandsMap SubCommandFuncsMap) error {
	...
	if t.GetCurrentCommand().HasSubCommand() {
		// check if that sub command has been registered on the route
		if subCommandFunc, exists := subCommandsMap[t.GetCurrentCommand().SubCommand().Name()]; exists {
	...
	if t.GetCurrentCommand().Flag('s') {
		helloStr = "Hi"
	}

	// check for 'name' arg
	if listOfNames := t.GetCurrentCommand().Arg("names"); listOfNames != nil {
...
func HelloSubCommandsMap() map[string]CommandFunc {
	...
	m["from"] = func(t *Helm, subCommandsMap SubCommandFuncsMap) error {
		...
		// check for 's' (short) flag
		if t.GetCurrentCommand().SubCommand().Flag('s') {
			helloStr = "Hey"
		}
		...
		// check for 'name' arg
		if listOfNames := t.GetCurrentCommand().SubCommand().Arg("names"); listOfNames != nil {
			helloStr += strings.Join(listOfNames, ", ")
		} else {
